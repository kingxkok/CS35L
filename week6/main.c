...
int nthreads; //declare nthreads outside so render() can use it
scene_t scene; //same with scene
...
void* //function to be multithreaded
render_(void *thread_num){ //need thread num to divide the forloop up
    int threadnum = *((int*)thread_num);
    Vec3 camera_pos;
    ...
    for( int px=start; px<end; ++px ) //adjacency helps with spatial locality,
    {                                   //so split into threads operating on contiguous chunks
    ...
    return NULL; //must return something for pthread
}
...
main( int argc, char **argv )
{
  ...
  printf( "# Rendering scene with %d spheres and %d lights\n",
    ...
    //rendering and printing to disk
    pthread_t *threads = (pthread_t*)malloc(nthreads*sizeof(pthread_t));
    int *thread_id = (int*)malloc(nthreads*sizeof(int)); //store thread IDs

    for(int t = 0; t < nthreads; t++)
    {
        thread_id[t] = t;
        int ret = pthread_create(&threads[t], NULL, render_, &thread_id[t]);
        if(ret)
        {
